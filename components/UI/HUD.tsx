...
const VirtualControls: React.FC = () => {
    const { hasImmortality, isImmortalityActive, activateImmortality } = useStore();
    
    const triggerMove = (dir: 'left' | 'right') => {
        window.dispatchEvent(new CustomEvent('input-move', { detail: { dir } }));
    };

    const triggerJump = () => {
        window.dispatchEvent(new CustomEvent('input-jump'));
    };

    return (
        <div className="absolute inset-0 pointer-events-none z-40 flex flex-col justify-end p-6 md:hidden">
            <div className="flex justify-between items-end w-full pb-4">
                {/* Left side: Navigation */}
                <div className="flex space-x-4 pointer-events-auto">
                    <button 
                        onPointerDown={() => triggerMove('left')}
                        className="w-16 h-16 rounded-2xl bg-white/5 border border-white/20 backdrop-blur-md flex items-center justify-center active:scale-90 active:bg-cyan-500/20 active:border-cyan-400 transition-all shadow-lg"
                    >
                        <ChevronLeft className="w-8 h-8 text-white" />
                    </button>
                    <button 
                        onPointerDown={() => triggerMove('right')}
                        className="w-16 h-16 rounded-2xl bg-white/5 border border-white/20 backdrop-blur-md flex items-center justify-center active:scale-90 active:bg-cyan-500/20 active:border-cyan-400 transition-all shadow-lg"
                    >
                ...
                <div className="flex flex-col items-center space-y-4 pointer-events-auto">
                    {hasImmortality && (
                        <button 
                            onPointerDown={() => activateImmortality()}
                            className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${isImmortalityActive ? 'bg-yellow-500 shadow-[0_0_20px_gold]' : 'bg-white/5 border border-yellow-500/40'}`}
                        >
                            <Shield className={`w-6 h-6 ${isImmortalityActive ? 'text-black' : 'text-yellow-500'}`} />
                        </button>
                    )}
                    <button 
                        onPointerDown={() => triggerJump()}
                        className="w-20 h-20 rounded-full bg-gradient-to-tr from-cyan-600 to-blue-500 border-2 border-white/30 flex items-center justify-center active:scale-95 active:brightness-125 transition-all shadow-[0_0_20px_rgba(0,255,255,0.3)]"
                    >
  ...
  return (
    <>
        <div className={containerClass}>
            ...
            <div className="w-full flex justify-end items-end pb-24 md:pb-0">
                <div className="flex items-center space-x-2 text-cyan-500 opacity-70">
                    <Zap className="w-4 h-4 md:w-6 md:h-6 animate-pulse" />
                    <span className="font-mono text-base md:text-xl uppercase">SPD {Math.round((speed / RUN_SPEED_BASE) * 100)}%</span>
                </div>
            </div>
        </div>
        <VirtualControls />
