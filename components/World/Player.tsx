...
export const Player: React.FC = () => {
  ...
  // Keyboard and Custom Events for Virtual Controls
  useEffect(() => {
    ...
    const handleInputMove = (e: any) => {
        if (status !== GameStatus.PLAYING) return;
        const maxLane = Math.floor(laneCount / 2);
        if (e.detail.dir === 'left') setLane(l => Math.max(l - 1, -maxLane));
        if (e.detail.dir === 'right') setLane(l => Math.min(l + 1, maxLane));
    };
    ...
    window.addEventListener('input-move', handleInputMove);
    window.addEventListener('input-jump', handleInputJump);
    return () => {
        window.removeEventListener('keydown', handleKeyDown);
        window.removeEventListener('input-move', handleInputMove);
        window.removeEventListener('input-jump', handleInputJump);
    };
  }, [status, laneCount, hasDoubleJump, activateImmortality]);

  // Swiping support for full-screen gestural backup
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        if (status !== GameStatus.PLAYING) return;
        // Don't trigger swipe if a button was likely the target (handled by HUD)
